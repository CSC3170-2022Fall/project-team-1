...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    while ($appointed_operation = mysqli_fetch_array($appointed_operations)) {
        ...
        if (!empty($available_machine)) {
            $priorities = $mysqli->query("SELECT `priority` FROM Operation_Types WHERE `chip_model` = '$appointed_operation[chip_model]' AND `operation_type` = '$appointed_operation[operation_type]'");
            $priority = mysqli_fetch_array($priorities);
